...
def create_custom_theme():
    ...
    pio.templates['custom'] = dict(
        layout=go.Layout(
            colorway=[THEME['line_chart_color']],              #set the line chart color
            dragmode=False,
            colorscale=dict(sequential=THEME['colorscale']),   #set the heatmap color 
            coloraxis=dict(autocolorscale=True),               #set the heatmap color
            xaxis=dict(tickangle=-45),                         #x axis ticks are titlted 45 to right
