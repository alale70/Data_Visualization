...
def to_df(data):
    ...
    # creating the dataset with specific columns name
    m_features = data["features"]
    for i, el in enumerate(m_features):
        for key in el["properties"]:          # extract each key
            el["properties.{}".format(key)] = el["properties"][key] # rename colum
        del el["properties"]
        for key in el["geometry"]:
            el["geometry.{}".format(key)] = el["geometry"][key]
        del el["geometry"]

    my_df = pd.DataFrame(m_features) # convert to data frame
...
def update_titles(my_df):
    ...
    # replace the specific column with name in 'TITLES' dictionary (removing number)
...
def sort_df(my_df):
    ...
    '''
    # TODO : Sort the df
    # sorting the data frame by indexing specific column
    my_df = my_df.set_index('properties.TYPE_SITE_INTERVENTION')
...
def get_neighborhoods(montreal_data):
    ...
    # extract the key [NOM] as a name of neighborhood
    m_features = montreal_data["features"]
    properties = [x['properties'] for x in m_features]
    name = [x['NOM'] for x in properties]
    return name
