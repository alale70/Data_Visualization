...
def prep_data():
    ...
    '''
    dataframe = pd.read_csv('romeo_and_juliet.csv')
...
def radio_updated(mode, figure):
    ...
    #updating the bar chart based on the current mode
    new_fig = bar_chart.draw(new_fig, data, mode)
    #updating the y axis of the bar chart based on the current mode
    bar_chart.update_y_axis(new_fig, mode)
    #updating the information text at the left of the footer based on the current mode
    label_elements = [html.Span(mode , style={'fontWeight': 'bold'})]
    return new_fig, label_elements
